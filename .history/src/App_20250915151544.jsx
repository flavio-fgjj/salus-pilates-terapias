...
function App() {
  const [isPromoBoxVisible, setIsPromoBoxVisible] = useState(true);
  return (
    <div>
      <Header />
      ...
      {/* Box de Promoção */}
      {isPromoBoxVisible && (
        <div className="fixed bottom-6 left-4 right-4 sm:left-6 sm:right-auto sm:max-w-xs bg-gradient-to-br from-[#B7D0CA] to-[#9BC4B8] rounded-xl shadow-2xl border border-white/20 p-4 z-50 animate-pulse">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className="font-bold text-white text-sm mb-1">Promoção Especial!</h4>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-block bg-white hover:bg-white/90 text-[#B7D0CA] text-xs font-semibold px-3 py-1.5 rounded-md transition"
            >
              Quero aproveitar!
            </a>
          </div>
          <button
            onClick={() => setIsPromoBoxVisible(false)}
            className="flex-shrink-0 w-6 h-6 rounded-full bg-white/20 hover:bg-white/30 flex items-center justify-center transition"
            aria-label="Fechar promoção"
          >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3 text-white">
      ...
      {/* Botão WhatsApp - só aparece quando o box promocional está fechado */}
      {!isPromoBoxVisible && (
        <a
